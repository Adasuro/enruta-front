...
const NavSectionLabel: React.FC<{ title: string, show: boolean, isCollapsed: boolean, onToggle: () => void }> = ({ title, show, isCollapsed, onToggle }) => {
  ...
  return (
    ...
    >
      <div className="text-[0.65rem] font-extrabold text-primary-200/50 uppercase tracking-widest whitespace-nowrap overflow-hidden text-ellipsis">
        {title}
      </div>
      <ChevronDown size={12} className={`text-primary-200/50 transition-transform duration-200 shrink-0 ${isCollapsed ? '-rotate-90' : ''}`} />
    </div>
  );
};
  ...
  return (
    <>
      ...
      <motion.aside 
        initial={false}
        animate={{ 
          width: isMobile ? '280px' : (isExpanded ? '16.5rem' : '4.5rem'),
        ...
        className="fixed top-0 left-0 h-screen bg-primary-900 text-white flex flex-col z-[1001] shadow-lg"
      >
        ...
        {isMobile && (
            <div className="absolute top-4 -right-12">
                <button 
                    onClick={() => setSidebarOpen(false)}
                    className="w-10 h-10 rounded-full bg-primary-800 text-white flex items-center justify-center shadow-lg border-none cursor-pointer"
                >
                    <X size={20} />
                </button>
            </div>
        ...
        <header className="shrink-0 border-b border-white/5">
            <div className="p-5 flex justify-center max-md:pt-8">
            ...
            <div className="p-4 relative" ref={menuRef}>
                ...
                {showBusinessMenu && !isSuperAdmin && (
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className="absolute top-[calc(100%+0.25rem)] left-2 right-2 bg-white rounded-lg shadow-xl overflow-hidden z-[110] text-gray-900 border border-gray-200"
                    >
        ...
        <nav 
            className="flex-1 py-4 px-2 flex flex-col gap-1 overflow-y-auto overflow-x-hidden pointer-events-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10 hover:scrollbar-thumb-white/20"
        >
            <div>
        ...
        {/* 3. FOOTER (Fixed) */}
        <footer className="shrink-0 p-2 border-t border-white/5 flex flex-col gap-1 pointer-events-auto mb-2">
            <NavItem to="/console/settings" icon={<Settings size={20} />} label="Configuración" isExpanded={isExpanded || isMobile} />
            <button 
            ...
            {(isExpanded || isMobile) && <span className="text-sm">Cerrar Sesión</span>}
            </button>
        </footer>
      </motion.aside>
    </>
